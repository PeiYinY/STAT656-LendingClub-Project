...
Note: (by Pei)
 - would annual_inc, zipcode measure the same underlying phenomenon?
...
LCDataTB_Seb <-  LCDataTB %>% select(where(~n_distinct(.) > 1)) #(Sebastian)

# Remove features that have close to zero variance
LCDataTB_Pei <- LCDataTB[, -nearZeroVar(LCDataTB)] #(Pei)
```
